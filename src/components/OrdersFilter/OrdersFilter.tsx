...
const OrdersFilter = (props: OrdersFilterProps) => {
   ...
   const applyFilter = async () => {
      ...
      if(
         ...
         !!searchValue ||
         clientList.length !== dataClientList?.length ||
   ...
   useEffect(() => {
      const listener = (e: any) => {
         ...
         (clientList.length !== dataClientList?.length && enter) ||
                    ...
                    <Col grid={{ md: 3, xs: 6 }}>
                     <div className={styles.filterInputClients} style={{marginBottom: '0'}}>
                        <DropDownFilter
                           ...
                           onChange={(values) => { setClientList(values) }}
                           fetchStatuses={{ isLoading: isLoadingClientList, isError: isErrorClientList }}
                           title={clientList?.length == 1 ?
                              dataClientList?.map(item => (
                                 item.value === clientList[0] && item.label as string
                              ))
                                 
                              : !clientList?.length ? 
                                 `[ 0 ] ${t('clients')}` : t('clients')
                           }
                           data={dataClientList ? dataClientList : []}
